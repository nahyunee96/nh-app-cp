...
import PopupContent from './term.popup';
...
function Join () {
    useEffect(() => {
        if (typeof window !== 'undefined') {
            ...
            else if (JoinFormPwCfInput.value === '') {
                alert('비밀번호를 한번 더 입력해주세요.')
                JoinFormPwCfInput.focus()
            } else if (JoinPwInputV !== JoinPwCfInputV) {
                alert('비밀번호 확인이 일치하지 않습니다. 다시 입력해 주세요.')
            }

            //name
            else if (JoinFormNameInput.value === '') {
                alert('성명을 입력해주세요.')
                JoinFormNameInput.focus()
            }

            //email
            else if (JoinFormMailInput.value === '') {
                alert('이메일을 입력해주세요.')
    ...
    return (
        ...
        '>
            ...
            <div className={`mb-16 flex justify-between`}>
                ...
                <div className="">
                    <button type="button" id="popupDom"  onClick={this.openPopup} className='text-slate-700 cursor-pointer underline hover:text-slate-300 duration-300'>개인정보 처리방침 보기</button>
